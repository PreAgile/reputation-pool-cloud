...
export const ko: Dict = {
  ...
  docs: {
    ...
    items: [
      ...
      { tag: "가이드", title: "핵심 개념", body: "평판, 쿨다운, 컨텍스트, 리스 — 엔진이 실제로 판단하는 방식.", go: "모델 이해하기 →" },
    ],
  ...
  status: {
    meta: {
      title: "서비스 상태 — reputation·pool",
      description:
        "지금 기록된 사고가 열려 있는지, 그리고 지금까지 적어 둔 사고 로그. 보고 대상인 앱 서버와 분리된 Cloudflare Pages 에서 서빙됩니다.",
    },
    heading: "서비스 상태",
    lead: "이 페이지는 보고 대상인 앱 서버가 아니라 Cloudflare Pages 에서 나갑니다 — 정작 이 페이지가 필요해지는 날에도 살아 있어야 하기 때문입니다.",

    state: {
      label: "지금",
      none: "진행 중으로 기록된 사고가 없습니다",
      degraded: "성능 저하가 진행 중입니다",
      outage: "장애가 진행 중입니다",
      derivedFrom:
        "이 한 줄은 아래 사고 로그에서 나옵니다. 로그는 사람이 손으로 적으므로, 이 문장은 '아무도 열린 사고를 적지 않았다'는 뜻이지 '방금 기계가 서비스를 확인했고 멀쩡하더라'는 뜻이 아닙니다.",
    },

    uptime: {
      heading: "업타임",
      absent:
        "여기에는 가용률 수치가 없습니다. 아직 그것을 재는 것이 없기 때문입니다. 관측하지 않은 숫자는 측정이 아니라 주장이라, 정직하게 채울 수 있을 때까지 자리를 비워 둡니다.",
      rejected:
        "한 가지는 이미 탈락했습니다 — 엔진이 내보내는 Prometheus 지표를 재사용하는 방법입니다. 그 지표는 앱 서버가 만들기 때문에, 정작 장애로 읽을 가치가 생기는 순간에 함께 꺼집니다.",
      next: "이 자리에 들어갈 것은 우리 인프라 바깥에서 찔러 보는 관측입니다 — 시작은 UptimeRobot 같은 외부 모니터, 이력을 우리 것으로 갖고 싶어지면 cron 으로 도는 Cloudflare Worker 와 KV 입니다. 둘 다 우리 서버가 멈춘 동안에도 계속 봅니다.",
    },
    ...
    log: {
      ...
      empty: "기록된 사고가 없습니다.",
      scope:
        "사고는 생길 때마다 사람이 손으로 적습니다. 그래서 이 로그에는 저희가 알아채고 적어 둔 것만 있고, 위 날짜 이전의 일은 들어 있지 않습니다.",
      sinceLabel: "기록 시작",
      ongoing: "진행 중",
      resolved: "해소됨",
      startedLabel: "발생",
      endedLabel: "종료",
      durationLabel: "지속",
      severity: { outage: "장애", degraded: "성능 저하" },
    },

    independence: {
      heading: "이 페이지가 다른 곳에 있는 이유",
      body: "대시보드와 API 는 저희 서버에서 돕니다. 이 페이지는 뒤에 서버가 없는 Cloudflare Pages 의 정적 파일이라, 그쪽이 죽어도 함께 죽지 않습니다. 이 분리가 전부입니다 — 감시 대상 위에 올려 둔 상태 페이지는 필요 없을 때만 대답합니다.",
    },

    report: {
      heading: "여기 없는 문제를 겪고 계신가요?",
      body: "로그에는 저희가 알아챈 것만 있습니다. 목록에 없는 문제를 겪고 계시면 알려 주세요 — 실제로 그렇게 알게 되는 경우가 많습니다.",
      cta: "이메일 보내기",
    },
  },
  ...
  footer: {
    columns: [
      { heading: "제품", links: ["기능", "동작 방식", "문서", "상태"] },
