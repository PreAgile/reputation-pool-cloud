...
> **⚠️ 근거 범위(정직하게).** 이 문서의 Cloudflare 관련 서술은 **로그인된 실제 대시보드를 라이브로 관찰한 것이
> 아니다.** 분석 환경의 Playwright는 사용자의 브라우저 세션(쿠키)에 접근할 수 없고, 남의 자격증명으로 로그인
> 하지 않는다. 따라서 Cloudflare 부분은 **널리 알려진 SaaS 운영 콘솔의 관례적 패턴 지식**에 기반하며(Cloudflare가
> 그 대표적 사례), 특정 픽셀 값·현재 DOM·정확한 문구는 **추정**으로 표시한다. 반대로 **reputation-pool-cloud의 API·
> 데이터 모델·현재 화면 구성은 이 저장소에서 직접 확인한 사실**이다.
>
...
| API key 테이블 | 라벨·prefix(mono 마스킹)·생성일·상태 + 발급 원문 1회 박스 | 보안 UX |
| event 필터 | 유형·종류·텍스트(+기간) — 서버 필터는 후속(#30/#29) | 큰 목록 좁히기 |
