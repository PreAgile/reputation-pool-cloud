...
- **admin** (`reputation-pool.admin.username/password`) — 운영자, 하나뿐이다. 토큰에 `scope=admin` 이
  실리고, 리소스 차단·해제, API 키 발급·폐기, 테넌트 생성·정지·삭제를 할 수 있는 유일한 계정이다.
- **viewers** (`reputation-pool.admin.viewers[]`) — 열람 전용, 여러 개. 각 토큰에 `scope=viewer` 가
  실리고 `/api/**` 의 `GET`·`HEAD` 만 통과한다. 선택 설정이며, 비어 있으면 뷰어 토큰은 발급되지 않는다.
  항목마다 `tenant` 를 따로 줄 수 있고, 비우면 어드민 `tenant` 를 따른다.

**왜 나눴나.** 콘솔을 팀 밖에 보여주려면(데모 계정) 자격증명이 공개된다. 공개된 자격증명이 상태를 바꿀 수
있으면 안 된다. 그래서 "보여주기"와 "바꾸기"를 계정 수준에서 가른다 — 공개의 대가를 *콘솔이 이미 표시하는
것의 노출*로 한정한다.

**왜 뷰어가 목록인가.** 보여줄 상대마다 계정을 따로 판다. 하나를 회수해도 나머지가 흔들리지 않고, 감사
기록의 `sub` 클레임만 봐도 어느 쪽 세션인지 구분된다 — 공용 데모 계정 하나면 둘 다 잃는다.

**표시 대상도 고를 수 있어야 한다.** 어드민 콘솔은 보통 실제로 운영 중인 테넌트를 가리키고, 그 읽기 모델에는
실제 리소스 식별자가 들어 있다. 쓰기를 막는 것과 별개로, 공개 계정이 그걸 보면 안 된다. 그래서 뷰어 항목마다
`tenant` 를 따로 지정해 시연용 데이터가 든 테넌트로 보낸다.
