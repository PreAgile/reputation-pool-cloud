...
- 색은 상태 표현, 인터랙션, 그리고 아래에 명시한 차트·브랜드 예외에만 사용한다.
  그 밖의 장식적 강조와 그라디언트는 금지한다.
...
**에러도 같은 규칙을 따른다.** 목록·인라인 에러 상태는 `EmptyState tone="error"`와 복구 CTA를 사용한다.
전역 에러 화면은 `error-screen.tsx`의 `actions` 슬롯으로 복구 경로를 제공한다. 회색 한 줄로 끝내지
않는다. 현재 혼재 상태는 위반 목록 V8·V9.

## 10. 속도가 기능이다

- 인터랙션은 즉각 반응한다. 서버 왕복을 기다리는 동안 화면이 멈춰 있으면 안 된다.
- **낙관적 갱신은 결과가 멱등적이고 안전하게 되돌릴 수 있는 뮤테이션에 기본이다.** UI 를 먼저 바꾸고,
  실패하면 되돌리고 토스트로 알린다. 토스트 인프라는 이미 있다(`toast.tsx:83-87`).
- **block/unblock, 테넌트 suspend/reactivate, API 키 폐기 같은 운영 제어 작업은 예외다.** 서버 응답이
  타임아웃돼도 클라이언트가 실패로 보고 롤백하면, 실제로는 반영된 차단이 화면엔 "미차단"으로 남아
  운영자를 오도한다 — 결과가 되돌릴 수 없거나(폐기) 비용이 큰(정지) 작업이라 낙관적 갱신이 안전하지
  않다. 즉시 피드백은 그대로 요구하되, 방식은 다르다: 누른 즉시 **pending 상태**를 보여주고, 서버
  확인 후 실제 상태로 반영하거나 실패 시 강제로 다시 조회한다.
...
위반은 V7(스켈레톤 없는 로딩)·V13(운영 제어 작업에 pending 상태 없음)·V16(상세 화면 폴링을 멈출 수 없음).
...
| V13 | 차단/해제(운영 제어 작업)에 pending 상태가 없음. 왕복을 기다리는 동안 아무 즉시 피드백도 없다가 완료돼야 반영 | `overview/page.tsx:181-207`, `resources/[kind]/[value]/page.tsx:193-220` | 10 | 후속 |
...
| V16 | `usePoll(30000)` 에 `enabled` 를 안 넘겨 항상 켜져 있음. 백그라운드 탭은 자동으로 건너뛰지만, 오버뷰·이벤트와 달리 사용자가 수동으로 멈출 UI 가 없다 | `resources/[kind]/[value]/page.tsx:187-190` | 10 | 후속 |
