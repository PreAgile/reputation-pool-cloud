...
> a 의 키로는 열리고 b 의 키로는 실패한다.
>
> 그래서 스크립트는 **인증서 파일마다 PEM 블록이 정확히 하나이고 그것이 인증서인지** 확인한다. 이
> 검사가 이어붙인 파일을 거부하는 유일한 방어선이다 — 암호화 후의 수신자 수 검사로는 잡히지 않는다
> (경로 1개 / 수신자 1명으로 일치해 버린다). 같은 검사가 **인증서 뒤에 개인키가 붙은 파일**도 거부한다:
> `openssl x509` 는 첫 블록만 읽으므로 그것만으로는 통과하고, 그러면 복호화 수단이 서버에 남아 이
> 설계의 전제가 깨진다.
>
> 수신자 수 검사는 다른 것을 막는다 — openssl 이 어떤 이유로든(버전 차이·인자 처리 변경) 준 인증서보다
> 적은 수신자를 넣은 경우다.
...
### 10-1. 데드라인 알림 (`deadline-notify.sh`)

오프사이트 백업(§8-1)·`.env` 암호화·호스트 이전 스크립트(§11)는 전부 **인스턴스가 사라지는 시나리오**를
위해 만들었다. 그런데 그 시나리오가 시작되는 날짜는 이미 정해져 있고(체험 크레딧 만료 → 30일 유예 → 삭제),
**그날이 다가온다는 것을 알려 주는 장치가 없었다.** 준비를 다 해 두고 실행 시점을 놓치는 것이 가장 아까운
실패다.

```bash
./scripts/deadline-notify.sh --list        # 설정된 데드라인과 남은 일수
./scripts/deadline-notify.sh --dry-run     # 오늘 무엇을 보낼지만
./scripts/deadline-notify.sh --install     # systemd 타이머(매일 09:00 UTC)
```

설정은 `~/.rp-deadline.env` 이고 형식은 `DEADLINE_<이름>=YYYY-MM-DD` 다:
...
**날짜를 코드에 박지 않는다.** 위 두 날짜는 지금 이 임시 호스트(§현재 상태 ①)의 사정이고, A1 을 확보해
이전하면 무의미해진다 — 그때는 설정에서 줄을 지우면 끝이다.

- **알림은 D-14 / D-7 / D-3 / D-1 / D-0 다섯 번.** 매일 보내면 그 메일을 읽지 않게 되고(§8-1 이 손 실행
  실패 메일을 끈 것과 같은 이유), 한 번만 보내면 그 한 통을 놓쳤을 때 끝이다. 지난 날짜는 조용하다.
- **`#174`(dead man's switch)와 다른 것이다.** #174 는 "서버가 **이미** 죽었다" 를 잡아야 하므로 외부에
  있어야 한다. 이쪽은 "죽을 **날짜**가 다가온다" 이고 달력만 있으면 되므로 서버 안에서 보내면 충분하다 —
  그래서 외부 의존이 없고 `notify-mail.py`(§8-1)를 그대로 쓴다.
- **`migrate-host.sh` 의 타이머 재설치 목록에는 넣지 않았다.** 이전이 성공하면 그 데드라인 자체가
  사라지므로, 새 호스트에 자동으로 따라가는 것이 오히려 틀린 알림을 만든다. 새 호스트에 다른 데드라인이
  생기면 그때 설정과 함께 설치한다.
- 알림 시점 로직은 실제로 그 날짜가 되어야만 드러나므로 `RP_DEADLINE_TODAY=YYYY-MM-DD` 로 "오늘" 을
  주입할 수 있고, **CI 가 다섯 시점과 사이 날짜를 매번 실행한다.** 그 훅이 있는 이유가 이 테스트다.
...
| 2 기동 | 신규 호스트: 볼륨 먼저 복원 → `bootstrap.sh` → DB 복원 → 타이머 설치·**검증** | 아니오 | 신규 호스트만 만진다 |
| 3 검증 | `--resolve` 로 엣지를 우회해 직접 TLS 검증 + 행수 대조 | 아니오 | — |
| 4 전환 | Cloudflare A 레코드 `content` 만 PATCH | **예** | `--rollback` (스냅샷) |
| 5 마무리 | **신규 타이머 가드** → 구 호스트 타이머 정지 → 컨테이너 정지(**삭제 아님**) | 아니오 | `--rollback` |
...
1. **신규 호스트의 타이머를 먼저 끈다.** 켜 둔 채로 두면 트래픽을 받지 않는 호스트의 정지된 DB 가 계속
   오프사이트 버킷에 최신 타임스탬프로 올라가, 버킷의 "가장 최신 덤프" 가 버려진 호스트의 것이 된다
   (`--verify-latest` 도 그것을 검증한다). 함정 4 와 같은 사고를 방향만 바꿔 되풀이하는 것이다.
2. 구 호스트에서 `bootstrap.sh` 를 다시 돌리고, Phase 5 가 껐던 유닛만 `old-timers.txt` 대로 되살린다.
3. `dns-before.json` 으로 A 레코드를 복원하고 엣지 경유 헬스를 확인한다.
4. **`phase1`~`phase5` 마커를 전부 지운다.** 롤백으로 구 호스트가 다시 쓰기를 받으므로 기존 덤프는 그
   순간 낡는다. 마커를 남기면 재시도가 덤프 생성과 행수 대조를 건너뛰고 곧장 전환으로 가서 **롤백 기간에
   들어온 데이터가 빠진 채 DNS 를 옮긴다.** 그래서 재시도는 항상 Phase 1(새 덤프)부터 다시 돈다.
...
**1-2. 부분 전환에서 스냅샷을 덮어쓰면 롤백 근거가 사라진다.**
`app` 은 PATCH 성공, `grpc` 는 실패 — Cloudflare API 의 일시적 실패 한 번으로 zone 이 반쯤 옮겨질 수 있다.
그 상태에서 같은 스냅샷 경로로 재실행하면 전환 대상에는 **아직 구 IP 인 레코드만** 들어오므로, 스냅샷을
새로 쓰면 먼저 옮겨진 `app` 을 되돌릴 근거가 없어진다(`--restore` 후에도 `app` 과 `grpc` 가 서로 다른
호스트를 가리킨다). 그래서 `cf-dns.sh --switch` 는 **스냅샷이 이미 있으면 덮어쓰지 않고 부분 전환 재개로
...
일어나 몇 주 뒤에 발견된다. Phase 5 는 타이머를 먼저 끄고 그 다음 컨테이너를 멈춘다. 끄기 **전에** 신규 호스트에서 배포·백업 타이머가 실제로 도는지
가드로 확인한다 — Phase 2 가 이미 검증하지만, 이전이 여러 번 재개될 수 있고 그 사이 신규 쪽 타이머가
죽었을 수 있다. 이 가드가 없으면 "양쪽 모두 배포·백업 없음" 이 성공으로 끝난다.
