...
손으로 하지 않는 이유는 재구축 시간이 아니라 **드리프트 복제**다. 지금 호스트는 `.env` 에
`PULL_DEPLOY_ENABLED=true` 가 있는데 `rp-pull-deploy.timer` 가 설치돼 있지 않아 도는 이미지가 레포 HEAD 보다
한 커밋 낡았다(§7-1 을 켰다고 **선언**만 된 상태). 손으로 옮기면 이 상태가 그대로 새 호스트에 복사된다.
그래서 이전 스크립트는 데이터만이 아니라 **호스트의 성질(타이머·하드닝)까지 복원**한다.
...
일어나 몇 주 뒤에 발견된다. Phase 5 는 `rp-pull-deploy.timer`·`rp-backup-offsite.timer`·`a1-hunter.service`
를 먼저 끄고 그 다음 컨테이너를 멈춘다.
