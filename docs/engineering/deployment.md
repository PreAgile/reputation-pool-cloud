...
1. **서버 체크아웃을 그 커밋으로 맞춘다** (`git fetch` + `git reset --hard <sha>`). 이미지 pull 만으로는
   ...
   대상이라 지워지지 않는다.
2. **이미지 태그를 그 커밋으로 고정해** `bootstrap.sh` 를 부른다(`APP_IMAGE_TAG=sha-<커밋>`).
   `latest` 를 쓰면 배포가 도는 사이 다음 머지로 태그가 옮겨가 "체크아웃 커밋 ≠ 실행 이미지" 조합이 될 수
   있다. `.env` 를 고치지 않고 환경변수로 넘긴다 — compose 가 `${APP_IMAGE_TAG:-latest}` 로 환경에서도
   보간하고, `.env` 는 운영자의 상태이므로 CI 가 건드릴 이유가 없다.
