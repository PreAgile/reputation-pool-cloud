...
- 클래스가 스레드 안전을 주장하면(javadoc·`Concurrent*` 자료구조·`synchronized`) 그 주장을 검증하는 동시성 테스트를 넣는다. 단일 스레드 테스트는 락을 통째로 지워도 전부 통과하므로 주장을 한 번도 건드리지 않는다.
  - 경쟁 시점을 **스케줄러 운에 맡기지 않는다.** `CountDownLatch` 2단계 랑데부(전원이 출발선에 서면 한 번에 푼다)나 barrier 로 동시 진입을 강제한다. 스레드 풀 크기는 태스크 수 이상이어야 랑데부가 교착되지 않는다.
  - 공유 상태의 **불변식**을 단정한다("총 허용은 정확히 burst"처럼 등식이 나오게 시나리오를 잡는다). 부등식(`<=`)밖에 못 쓰는 시나리오는 유실된 갱신을 놓친다.
  - 락을 지운 뒤 실제로 빨개지는지 보고, **정상 코드에서 여러 번 돌려 오탐이 없는지도** 확인한다. 경합 재현은 확률적이라 두 수치(탐지율·오탐율)를 함께 재야 판단할 수 있다.
...
- 테스트를 다 쓴 뒤 검증 대상 로직을 **일부러 깨뜨려** 그 테스트가 실패하는지 본다. 실패하지 않으면 그 테스트는 그 버그를 못 잡는 것이다. "충분히 테스트했나"를 의견이 아니라 확인 가능한 절차로 만든다.
  - 깨뜨린 내용을 기록하고, 확인 즉시 **원복한 뒤 전체 검증을 다시 돌리고** 커밋한다. 깨진 구현이 섞여 들어가는 사고를 막는다.
  - 결함 하나를 심어 잡혔다는 것은 **그 테스트가 그 버그를 잡는다**는 뜻일 뿐, 커버리지가 충분하다는 뜻이 아니다.
  - 테스트가 소스 밖의 파일(compose·설정·매니페스트)을 읽는다면 그 파일을 빌드 태스크의 **입력으로 선언**한다. 선언하지 않으면 그 파일만 바꾼 빌드가 `UP-TO-DATE`/`FROM-CACHE` 로 건너뛰어 조용히 초록색이 된다.
