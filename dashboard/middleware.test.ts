...
describe("로케일 미들웨어(#110 → #15): 실행 범위", () => {
  it("로케일이 응답을 바꾸는 경로에만 걸린다 → 이제 /login 하나뿐이다", () => {
    // 계층 분리(#15) 전에는 `/` 가 랜딩이라 방문자 신호로 언어가 갈렸다. 지금 `/` 는 `/overview` 로
    // 보내는 콘솔 진입점이라 갈릴 것이 없고, 랜딩의 언어 판별은 apex 쪽 Pages Functions 가 맡는다.
    expect(config.matcher).toEqual(["/login"]);
  });

  it("`/ko` 를 매처에 넣지 않는다 → 쿠키로 되돌리는 순간 리다이렉트 루프의 재료가 된다", () => {
