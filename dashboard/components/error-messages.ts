 ...
 *
 * 마케팅 사전(`components/marketing/i18n`)에 얹지 않은 이유: 그쪽은 랜딩 섹션 구조에 맞춰진 큰 타입이고
 * 서버 컴포넌트 전용(ReactNode 를 담는다)이다. 에러 문구는 `global-error.tsx` 처럼 **루트 레이아웃 밖**
 * 에서도 읽혀야 하므로, 의존이 적은 평범한 문자열 사전으로 따로 둔다.
 *
