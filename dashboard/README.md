...
# 2) 대시보드
cd dashboard
corepack enable                    # Node 16.13+ 내장. package.json packageManager로 pnpm 버전 고정
pnpm install
pnpm dev                           # http://localhost:3000
```

패키지 매니저는 **pnpm**(`packageManager` 필드로 버전 고정, `corepack`이 강제). `sharp`(Next 이미지 최적화 네이티브 바이너리)의 install 스크립트만 `pnpm-workspace.yaml`의 `allowBuilds`로 허용한다(pnpm 10+ 공급망 보안 기본값 = 빌드 스크립트 차단).
